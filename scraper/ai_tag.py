...
# Phase 2 (TECH_DEF §1.2): universal フラグ判定 (「規模不問」相当の明示)
UNIVERSAL_SIZE_KEYWORDS = [
    "規模不問", "規模を問わず", "規模制限なし", "全規模対象",
    "従業員の制限なし", "従業員数の制限なし", "事業者を問わない",
]
# 数値制限の検出パターン (key_warnings に追加 + 該当しうるすべて展開)
SIZE_NUMERIC_PATTERN = re.compile(r"(従業員|社員)?\s*([0-9０-９]{1,4})\s*[人名]\s*(以下|未満)")
...
スキーマ:
{
  ...
  "size_tags": ["個人事業主","小規模事業者","中小企業","大企業" のうち該当しうるすべて],
  "size_universal": false,
  "size_inferred": false,
...
size に関する重要ルール (TECH_DEF §1.2):
1. **「該当しうるすべての規模カテゴリ」を size_tags に列挙する**
   - 例: 「従業員50人以下、業種問わず」 → ["個人事業主","小規模事業者","中小企業"]
   - 例: 「中小企業対象」 → ["中小企業"] のみ (※包含関係は AI が展開しない)
2. 規模制限が中小企業基本法カテゴリと完全一致しない場合 (数値制限・年商制限など) → key_warnings に「○○以下に限定」を必ず追加
3. 「規模不問」「全規模対象」「事業者を問わない」明示 → size_universal=true, size_tags=[] (併存禁止)
4. 本文に規模記述なし → size_inferred=true (tagsは推測結果か空)
5. universal=true なら inferred=false (universal は明示の強いシグナル)
"""
...
def _heuristic_extract(record: dict) -> dict:
    ...
    if industry_universal:
        ...
        if not industries:
            ...
            industry_inferred = False
    # === size: 3-field (Phase 2, TECH_DEF §1.2) ===
    # 入力テキスト: 本文 + target_text (jGrants の構造化フィールド) を併用
    size_text = " ".join([text, record.get("target_text", "") or ""])
    size_universal = any(kw in size_text for kw in UNIVERSAL_SIZE_KEYWORDS)
    size_numeric_warning = None  # 数値制限検出時に key_warnings に追加する文字列
    if size_universal:
        # universal=true なら tags=[] (併存禁止、TECH_DEF ルール 3)
        sizes = []
        size_inferred = False
    else:
        # まず中小企業基本法カテゴリの明示を検出
        explicit_sizes = [k for k, ws in SIZE_KEYWORDS.items() if any(w in size_text for w in ws)]
        # 次に数値制限パターンを検出
        m = SIZE_NUMERIC_PATTERN.search(size_text)
        if m:
            num_str = m.group(2).translate(str.maketrans("０１２３４５６７８９", "0123456789"))
            try:
                num = int(num_str)
                # 該当しうるすべてを列挙: 個人事業主・小規模事業者・中小企業は X人以下 を満たしうる
                if num <= 5:
                    expanded = ["個人事業主", "小規模事業者"]
                elif num <= 20:
                    expanded = ["個人事業主", "小規模事業者", "中小企業"]
                elif num <= 300:
                    expanded = ["個人事業主", "小規模事業者", "中小企業"]
                else:
                    expanded = ["個人事業主", "小規模事業者", "中小企業", "大企業"]
                # 明示カテゴリと和集合
                sizes = list(dict.fromkeys(explicit_sizes + expanded))
                # 中小企業基本法カテゴリと完全一致しない場合 → key_warnings 追加
                size_numeric_warning = f"従業員{num}人{m.group(3)}に限定"
            except ValueError:
                sizes = explicit_sizes
        else:
            sizes = explicit_sizes
        # 何も検出できなければデフォルト + inferred=true
        if not sizes:
            sizes = ["中小企業"]
            size_inferred = True
        else:
    ...
    # === purpose: Phase 3 で更新予定、現状は旧ロジック維持 ===
    ...
    # Phase 2: size の数値制限を warning に追加 (TECH_DEF §1.2 ルール 2 / §1.7)
    if size_numeric_warning:
        warnings.append(size_numeric_warning)
...
ALLOWED_KEYS = {
    ...
    # Phase 2 (TECH_DEF §1.2) で追加
    "size_universal", "size_inferred",
}

# 9 カテゴリ allowlist (TECH_DEF §1.1)。「全業種」はタグ値ではない (universal で表現)
INDUSTRY_ALLOWLIST = {"IT", "製造業", "飲食", "農業", "建設", "小売", "医療", "教育", "その他"}
# 4 カテゴリ allowlist (TECH_DEF §1.2、中小企業基本法準拠)。「全規模」は universal で表現
SIZE_ALLOWLIST = {"個人事業主", "小規模事業者", "中小企業", "大企業"}
...
def _apply_result(record: dict, result: dict) -> None:
    ...
    for k in ALLOWED_KEYS:
        ...
        if k in {"concrete_targets", "eligible_expenses", "required_documents",
            ...
            # Phase 2: allowlist で filter、最大 4 個 (該当しうるすべて原則)
            if isinstance(v, list):
                cleaned = [clean_text(str(x), max_chars=80) for x in v if x]
                record[k] = [t for t in cleaned if t in SIZE_ALLOWLIST][:4]
            ...
            try:
                ...
                pass
    # Phase 1: universal=true なら tags=[] (TECH_DEF §1.1 ルール 5、AI が誤って併存させても補正)
    if record.get("industry_universal") is True:
        record["industry_tags"] = []
    # Phase 2: size も同様に併存禁止 (TECH_DEF §1.2 ルール 3)
    if record.get("size_universal") is True:
        record["size_tags"] = []
